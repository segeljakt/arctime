...
impl<I: DataReqs> Stream<I> {
    ...
    pub(crate) fn apply_terminating<S: DataReqs, O: DataReqs, R: DataReqs>(
        self,
        task: Task<S, I, O, R>,
    ) -> Stream<O> {
        task(self)
    ...
    /// Transform a stream into a new stream.
    pub(crate) fn apply<S: DataReqs, O: DataReqs>(self, task: Task<S, I, O, Never>) -> Stream<O> {
        task(self)
    }

    /// Transform two streams into one.
    pub(crate) fn merge<S: DataReqs, X: DataReqs, O: DataReqs>(
        self,
        other: Stream<X>,
    ...
    pub(crate) fn split<S: DataReqs, X: DataReqs, O: DataReqs>(
        self,
